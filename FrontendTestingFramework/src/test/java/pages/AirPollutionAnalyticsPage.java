 ...
 * #filterPollutionlevel select (All Levels | Low | Moderate | High | Severe)
 ...
 * td.col-co .co-value → co number
 * td.col-ozone .ozone-value → ozone number (formatted 1.1-1)
 * td .pollution-badge → pollution level
 ...
 *
 * Navigation:
 * a.back-link[routerLink="/air-pollution"]
 * a[routerLink="/home"].nav-link
 * a[routerLink="/air-pollution-alerts"].nav-link
 * button.logout-btn
 *
 ...
 */
public class AirPollutionAnalyticsPage extends BasePage {
    ...
    private static final By FILTER_CONGESTION = By.id("pollutionLevel");
    ...
    private static final By ROW_LOCATION_CELL = By.cssSelector("td:nth-child(2)");
    private static final By ROW_CO_CELL = By.cssSelector("td:nth-child(3)");
    private static final By ROW_OZONE_CELL = By.cssSelector("td:nth-child(4)");
    private static final By CONGESTION_BADGES = By.cssSelector(".pollution-badge");
    ...
    private static final By PREV_PAGE_BTN = By
            .xpath("//div[contains(@class,'pagination-controls')]//button[contains(text(),'‹')]");
    private static final By FIRST_PAGE_BTN = By
            .xpath("//div[contains(@class,'pagination-controls')]//button[contains(text(),'«')]");
    ...
    private static final By NAV_DASHBOARD = By.xpath("//a[@href='/air' and contains(@class,'nav-link')]");
    private static final By NAV_ALERTS = By.xpath("//a[@href='/air-alerts' and contains(@class,'nav-link')]");
    private static final By LOGOUT_BTN = By.cssSelector("button.logout-btn");

    // ── Constructor ───────────────────────────────────────────────────────────

    public AirPollutionAnalyticsPage(WebDriver driver) {
        super(driver);
    }
    ...
    public AirPollutionDashboardPage clickDashboardNav() {
    ...
    public AirPollutionAnalyticsPage enterDateFrom(String dateFrom) {
        setDateInput(FILTER_FROM, dateFrom);
        return this;
    }

    @Step("Enter end date: {dateTo}")
    public AirPollutionAnalyticsPage enterDateTo(String dateTo) {
        setDateInput(FILTER_TO, dateTo);
        return this;
    }

    @Step("Enter location filter: {location}")
    public AirPollutionAnalyticsPage enterLocation(String location) {
    ...
    @Step("Select pollution level: {level}")
    public AirPollutionAnalyticsPage selectPollutionlevelLevel(String level) {
        new Select(wait.waitForVisible(FILTER_CONGESTION)).selectByVisibleText(level);
        return this;
    }

    @Step("Select sort option: {sortValue}")
    public AirPollutionAnalyticsPage selectSort(String sortValue) {
        new Select(wait.waitForVisible(FILTER_SORT)).selectByValue(sortValue);
        return this;
    }

    @Step("Select page size: {size}")
    public AirPollutionAnalyticsPage selectPageSize(String size) {
    ...
    public AirPollutionAnalyticsPage clickApply() {
    ...
    public AirPollutionAnalyticsPage clickReset() {
    ...
    public AirPollutionAnalyticsPage clickNextPage() {
    ...
    public AirPollutionAnalyticsPage clickPreviousPage() {
    ...
    public AirPollutionAnalyticsPage clickFirstPage() {
    ...
    public boolean isOnAnalyticsPage() {
        return urlContains("/air-analytics");
    }
    ...
    public boolean isTableLoadingDisplayed() {
        return isDisplayed(TABLE_LOADING);
    }
    ...
    public boolean isActiveFilterBadgeDisplayed() {
        return isDisplayed(ACTIVE_FILTER_BADGE);
    }
    ...
    public List<WebElement> getPollutionlevelBadges() {
        return driver.findElements(CONGESTION_BADGES);
    }

    public String getRecordCountText() {
        try {
            return wait.waitForVisible(RECORD_COUNT).getText().trim();
        } catch (Exception e) {
            return "";
        }
    }
    ...
    public String getActiveFilterBadgeText() {
        try {
            return wait.waitForVisible(ACTIVE_FILTER_BADGE).getText().trim();
        } catch (Exception e) {
            return "";
        }
    }
    ...
    public String getPaginationInfoText() {
        try {
            return wait.waitForVisible(PAGINATION_INFO).getText().trim();
        } catch (Exception e) {
            return "";
        }
    }
    ...
    @Step("Read first row co")
    public String getFirstRowCarbonMonoxide() {
        wait.waitForPresenceOfAll(TABLE_ROWS);
        return wait.waitForVisible(FIRST_ROW_CO).getText().trim();
    }

    @Step("Read first row ozone")
    public String getFirstRowOzone() {
        wait.waitForPresenceOfAll(TABLE_ROWS);
        return wait.waitForVisible(FIRST_ROW_OZONE).getText().trim();
    }

    @Step("Read first row pollution level")
